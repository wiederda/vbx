...
func CheckUnknownCalls(stmts []Stmt) []CallWarning {
	c := &callChecker{known: knownNameSet()}

	topLevelVars := make(map[string]bool)
	collectDeclaredNames(stmts, topLevelVars)

	c.walkStmts(stmts, scope{context: "Top-Level", vars: topLevelVars})
	...
	for _, sn := range subNodes {
		...
		c.walkStmts(sn.Body, scope{context: "Sub " + sn.Name, vars: merged})
	}

	for _, fn := range funcNodes {
		procVars := collectProcNames(fn.Params, fn.Body)
		merged := mergeVarSets(topLevelVars, procVars)
		c.walkStmts(fn.Body, scope{context: "Function " + fn.Name, vars: merged})
	}
...
func (c *callChecker) reportIfUnknown(name string, sc scope) {
	...
	if sc.vars[low] {
		// z.B. "sections" - deklarierte Variable, Array-/Map-Zugriff
		return
	}

	// Punktbehafteter Name (z.B. "meinObjekt.Feld"): Falls der Teil vor
	// dem ersten Punkt eine deklarierte Variable ist, kann es sich um
	// Objekt-Feld-Zugriff auf ein Array/eine Map handeln - das Kind
	// ist wie bei "sections(i)" erst zur Laufzeit bekannt, daher
	// ebenfalls keine Warnung. Trifft NICHT auf echte Modul-Aufrufe wie
	// "ini.Sections" zu, weil "ini" selbst nie eine deklarierte
	// Variable ist (Module werden nicht per Dim/Public angelegt).
...
func (c *callChecker) walkStmt(s Stmt, sc scope) {
	...
	case *SubNode:
		// wird separat mit eigenem Scope behandelt (CheckUnknownCalls) -
		// hier nicht nochmal absteigen, sonst falscher Kontext/Scope.
	...
	default:
		...
		fmt.Printf("[PRECHECK] Unbekannter Stmt-Typ im Walker: %T (bitte in walkStmt ergänzen)\n", s)
	}
}
...
func (c *callChecker) walkExpr(e Expr, sc scope) {
	...
	case Value:
		// Literal (z.B. BOOL true/false aus parseFactor) - keine Kinder
	...
	case *MultiStmtNode:
		// laut evalExpr auch als Expr genutzt (verkettete Statements
		// innerhalb eines Ausdrucks)
		c.walkStmts(n.Stmts, sc)

	default:
		fmt.Printf("[PRECHECK] Unbekannter Expr-Typ im Walker: %T (bitte in walkExpr ergänzen)\n", e)
	}
}
