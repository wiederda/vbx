...
func InitArrayFunctions() {
	...
	Register(ns+"SortBy", "array", "arr, pattern, [desc]", "Sortiert ein Array von Strings anhand eines per Regex extrahierten Schlüssels (erste Capture Group). Gibt eine Kopie der Original-Elemente in sortierter Reihenfolge zurück. Elemente ohne Treffer werden immer ans Ende sortiert (unabhängig von der Richtung). Standard: aufsteigend (A-Z).", func(args []Value) Value {
		...
		if err != nil {
			return ErrorVal("ungültiges Regex-Pattern: " + err.Error())
		}

		desc := false
		if len(args) >= 3 {
			desc = isTruthy(args[2])
		}

		res := cloneArray(args[0].Arr)

		extractKey := func(v Value) (string, bool) {
			match := re.FindStringSubmatch(ToString(v))
			if len(match) < 2 {
				return "", false
			}
			return match[1], true
		...
		sort.SliceStable(res, func(i, j int) bool {
			keyI, okI := extractKey(res[i])
			keyJ, okJ := extractKey(res[j])

			// Elemente ohne Treffer landen immer am Ende, unabhängig von der Richtung.
			if !okI && !okJ {
				return false
			}
			if !okI {
